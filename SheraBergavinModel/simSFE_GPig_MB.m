...
Nfibers = 1;
fiber_freqs = fmin:fmax/Nfibers:fmax;
...
freqs = linscale(fmin,fmax,Nfreqs);
        ...
        for cnt1 = 1:Nfibers
            f = fiber_freqs(cnt1);
            mFiber(:,cnt1) = generate_moc_fiber1(cnt1);
        end
...
%                 xlim([0 1]);
        ...
        Data{nr} = D;
%         saveas(Nfig, num2str(Nreps), 'png');
%         saveas(Nreps, num2str(Nreps), 'fig')
    end
end
